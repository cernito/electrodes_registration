function pcFinalElectrodePositions = refine_alignment_combined(scan, pcElectrodes_aligned)
    ...
    if curvature_on_whole_head
        ...
        expectedClusters = 122; % Target number of clusters
        ...
        validPointsGC = vertcat(validCentroids{:});
        ...
        if numValidClusters > 0
           scatter3(validPointsGC(:, 1), validPointsGC(:, 2), validPointsGC(:, 3), ...
        ...
        for clusterID = uniqueClusters'
            ...
            if clusterDiameter < 3
                otherClusterPoints = selectedPoints(idx ~= -1 & idx ~= clusterID, :);
                if min(pdist2(clusterPoints, otherClusterPoints)) > 2
                    validClusters{end+1} = mean(clusterPoints,1);
                end
            end
        ...
        numValidClusters = numel(validClusters);
        validPointsMC = vertcat(validClusters{:});
        validClustersMC = validClusters;
        disp(['Number of Valid Clusters: ', num2str(numValidClusters)]);
        ...
        % Plot each valid cluster
        if numValidClusters > 0
           scatter3(validPointsMC(:, 1), validPointsMC(:, 2), validPointsMC(:, 3), ...
                    30, pointColors, 'filled');
        end
        ...
        disp('Validating GC clusters with MC clusters...');
        validClusters = {};
        
        % Validate clusters
        validationRadius = 5;
        
        numPoints = min(size(validPointsMC,1), size(validPointsGC,1));
        
        if size(validPointsMC,1) >= size(validPointsGC,1)
            candidatePoints = validPointsMC;
            validationPoints = validPointsGC;
        else
            candidatePoints = validPointsGC;
            validationPoints = validPointsMC;
        end
        
        for i = 1:numPoints
            
            candidatePoint = candidatePoints(i,:);
        
            % Query KD-tree for the nearest low-curvature cluster
            [idxNearest, distNearest] = knnsearch(validationPoints, candidatePoint);
        
            % Check if a nearby low-curvature cluster exists within the validation radius
            if distNearest <= validationRadius
                % Cluster is valid
                validClusters{end+1} = mean([candidatePoint; validationPoints(idxNearest,:)],1);
            end
        end
        ...
        if ~isempty(validClusters) && numValidClusters > 17
            ...
            % Use pdist2 to find the closest points on the scan
            % scan.Points contains all points in the scan
            closestPointIndices = knnsearch(scan.Points, validPoints);
            closestPointIndices

            % Map valid points back to the original vertex indices
            validIndices = false(size(scan.Points, 1), 1);
            validIndices(closestPointIndices) = true;
            
            % Update the selectedVertices mask
            selectedVertices = validIndices;
            size(selectedVertices)
            sum(selectedVertices == 1)
            size(scan.Points)
        else
            validPoints = validPointsMC;
            validClusters = validClustersMC;
            numValidClusters = numel(validClusters);
        ...
        leftMask = sortedTemplateElectrodes(:,1) < (min_x + (0.5)*x_size);
        mid_x_mask = sortedTemplateElectrodes(:,1) >= (min_x + (1/3)*x_size) & sortedTemplateElectrodes(:,1) < (min_x + (2/3)*x_size);
        rightMask = sortedTemplateElectrodes(:,1) > (min_x + (0.5)*x_size);
        
        top_z_threshold = (max(segmented_cap.Points(:,3)) - min(segmented_cap.Points(:,3))) * (2/3);
        topMask = sortedTemplateElectrodes(:,3) > top_z_threshold;
        
        min_y = min(segmented_cap.Points(:,2));
        y_size = (max(segmented_cap.Points(:,2)) - min_y);
        backMask = sortedTemplateElectrodes(:,2) < (min_y + (1/3)*y_size);
        mid_y_mask = sortedTemplateElectrodes(:,2) >= (min_y + (1/3)*y_size) & sortedTemplateElectrodes(:,2) < (min_y + (2/3)*y_size);
        frontMask = sortedTemplateElectrodes(:,2) > (min_y + (2/3)*y_size);
    ...
    for i = 1:size(templateElectrodes, 1)
        ...
        if any(localIdx)
            ...
            if (maxMCValue > nearestPointMC) || (minMCValue < -0.3)
    ...
    writeElectrodesToFcsv('final_electrode_positions.fcsv',labels,finalElectrodePositions);
    ...
    function visualize_final_electrode_positions()
        ...
        pcshow(finalElectrodePositions(betterMCpointIdx,:),[0.8500 0.3250 0.0980],'MarkerSize',600) 
