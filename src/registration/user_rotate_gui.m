function aligned_scan = user_rotate_ax(alignedToSphere, pcMri_model, ax)
    ...
    cla(ax);
    hold(ax, 'on');           % Keep the reference fixed in the plot
    pcshow(subsampledRefCloud.Location,'r', 'Parent', ax, 'MarkerSize', 40);  % Display the reference model
    h = pcshow(subsampledPtCloud.Location, 'g', 'Parent', ax, 'MarkerSize', 4);  % Display the point cloud with a handle
    xlabel('front');
    ylabel(ax, 'Y-axis');
    zlabel(ax, 'Z-axis');
    title(ax, 'Align the point cloud to match the reference');
    axis(ax, 'equal');
    rotate3d(ax, 'off');  % Disable interactive view rotation; we will rotate the point cloud instead
    ...
    while true
        ...
        cla(ax);  % Clear the current axes before plotting again
        pcshow(subsampledRefCloud.Location, 'r', 'Parent', ax, 'MarkerSize', 40);  % Plot the reference in red
        hold(ax, 'on');
        % Plot the moving subsampled point cloud with transparency
        pcshow(rotatedPoints, 'g', 'Parent', ax, 'MarkerSize', 4);  % Blue with transparency
        hold(ax, 'off');
        axis(ax, 'equal');
        xlabel(ax, 'front');
        % Set initial view to make the reference face forward (e.g., front view)
        view(ax, -6, 10);  % Set azimuth = 90 and elevation = 0 degrees for forward-facing view
        drawnow;
    end
